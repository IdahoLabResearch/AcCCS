...
class ServicePaymentSelectionReqState(AbstractState):
    ...
    def handlePacket(self, receivedPacket):
        ...
        rspPkt = V2G(self.emulator, self.emulator.EXIProcessor.encode(AuthorizationRequest(sessionID=self.emulator.sessionID)))
        return (AuthorizationReqState(self.emulator), StateMachineResponseType.SUCCESSFUL_TRANSITION, rspPkt)

class AuthorizationReqState(AbstractState):
    @property
    def validResponsePacketTypes(self) -> list:
        pkts = [PacketType.AuthorizationRes]
        return [pkt.value for pkt in pkts]
    
    @property
    def pktToSend(self) -> Packet | None:
        return V2G(self.emulator, self.emulator.EXIProcessor.encode(AuthorizationRequest(sessionID=self.emulator.sessionID)))
    
    @property
    def currentState(self) -> PacketType:
        return PacketType.AuthorizationReq
...
class ServicePaymentSelectionResState(AbstractState):
    @property
    def validResponsePacketTypes(self) -> list:
        pkts = [PacketType.AuthorizationReq]
        return [pkt.value for pkt in pkts]
    ...
    def handlePacket(self, receivedPacket):
        ...
        rspPkts = V2G(self.emulator, self.emulator.EXIProcessor.encode(AuthorizationResponse(sessionID=self.emulator.sessionID)))
        return (AuthorizationResState(self.emulator), StateMachineResponseType.SUCCESSFUL_TRANSITION, rspPkts)
    
class AuthorizationResState(AbstractState):
    @property
    def validResponsePacketTypes(self) -> list:
    ...
    @property
    def pktToSend(self) -> Packet | None:
        return V2G(self.emulator, self.emulator.EXIProcessor.encode(AuthorizationResponse(sessionID=self.emulator.sessionID)))

    @property
    def currentState(self) -> PacketType:
        return PacketType.AuthorizationRes
