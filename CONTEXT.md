...
The tree can address a **list-nested** wire field — a repeated child element emitted atomically inside a *single* message (e.g. DIN `ChargeParameterDiscoveryRes → SAScheduleList → SAScheduleTuple`) — written as a nested YAML list of maps mirroring the message model. The tree declares the *whole* list, its length included, so list cardinality is itself configurable (and fuzzable). A device file that redefines such a list restates it wholesale (lists replace, they do not index-merge). This is distinct from — and does not reopen — the rejected *per-instance-over-time* case (the same message type carrying a scripted sequence of values across successive emissions during a session), which remains disallowed: a list inside one message is a static payload, not behavior over time. See [ADR-0006](docs/adr/0006-message-field-tree-personality.md) (issue #81 amendment).

Validation is **path-strict, value-raw**: a field path must resolve to a real field in the message model (a typo is a hard error), but the value itself is not range- or enum-checked — illegal-but-encodable values are the point (red-team probing), bounded only by what the codec can serialize. **Caveat:** value-raw is *vacuous* for an enum-restricted wire field, where *illegal* (fails enum coercion) ⟺ *non-encodable* (the codec can only emit declared enum members) — there is no illegal-but-encodable value left to honor, so such a field is validated at personality *load* rather than honored raw (e.g. the enum *name* `DC_EXTENDED` written where the wire *value* `DC_extended` belongs, on `ServiceDiscoveryRes → ChargeService → EnergyTransferType`, is rejected up front). This is a corollary of value-raw, not a contradiction; see [ADR-0006](docs/adr/0006-message-field-tree-personality.md).
