...
class PhysicalValue(BaseModel):
    ...
    def validate_value_range(self) -> Self:
        ...
        """
        calculated_value = self.value * 10**self.multiplier
        if (
            0 < self._max_limit < calculated_value
            or calculated_value < self._min_limit < 0
        ):
            raise ValueError(
                f"{self.__name__[2:] }"  # type: ignore[attr-defined]
                f"value limit exceeded: {calculated_value} \n"
                f"Max: {self._max_limit} \n"
                f"Min: {self._min_limit}"
            )
        return self
...
class PVEVMaxCurrentLimitDin(PVEVMaxCurrentLimit):
    ...
    unit: Optional[Literal[UnitSymbol.AMPERE]] = Field(None, alias="Unit")
...
class PVEVMaxPowerLimitDin(PVEVMaxPowerLimit):
    ...
    unit: Optional[Literal[UnitSymbol.WATT]] = Field(None, alias="Unit")
...
class PVEVMaxVoltageLimitDin(PVEVMaxVoltageLimit):
    ...
    unit: Optional[Literal[UnitSymbol.VOLTAGE]] = Field(None, alias="Unit")
...
class PVRemainingTimeToFullSOCDin(PVRemainingTimeToFullSOC):
    ...
    unit: Optional[Literal[UnitSymbol.SECONDS]] = Field(None, alias="Unit")
