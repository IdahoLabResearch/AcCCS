...
- A **personality** (YAML) describes *who* the emulated device is. Per
  ...
  several protocols. DIN (both roles) and ISO-2 (SECC) are tree-backed with
  shipped baselines; the ISO-2 EVCC path is still pre-tree and ISO-20 has a
  registry but no baseline yet) —
