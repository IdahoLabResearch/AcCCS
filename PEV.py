...
class _TCPHandler:
    ...
    # def initNMAP(self):
    #     # This increments the filename for each nmap session
    #     fileList = glob("scan_results/scan_res_pev_[0-9][0-9][0-9].txt")
    #     maxFileNum = 0
    #     for fileName in fileList:
    #         num = int(fileName[-7:-4])
    #         maxFileNum = max(num, maxFileNum)
    #     self.nmapResFileName = "scan_results/scan_res_pev_{:0>3}.txt".format(maxFileNum + 1)

    #     self.portList = []
    #     # This text file contains the most common TCP ports to scan from nmap-services
    #     my_path = os.path.abspath(os.path.dirname("./"))
    #     print(my_path)
    #     with open("modportlist.txt", "r") as file:
    #         for line in file:
    #             self.portList.append(int(line.strip()))

    # def saveNMAPres(self):
    #     cutoff = -11
    #     if self.finishedNMAP:
    #         cutoff = len(self.portRes)
    #     with open(self.nmapResFileName, "a") as f:
    #         for e in self.portRes[:cutoff]:
    #             f.write("{: <5} | {}\n".format(e[0], e[1]))
    ...
    #     targetMAC = self.destinationMAC
    #     targetIP = self.destinationIP

    #     selfIP = self.sourceIP

    #     print(f"INFO (PEV) : Starting NMAP on port {self.portList[currentPort]} | {currentPort+1}/{maxPorts}")
    ...
    #         resp = srp1(
    #             Ether(src=self.sourceMAC, dst=targetMAC) / IPv6(src=selfIP, dst=targetIP) / TCP(sport=sport, dport=self.portList[i], flags="S"),
    #             timeout=1,
    #             verbose=0,
    #             iface=self.iface,
    #         )
    ...
    #         if resp == None:
    #             self.portRes.append((self.portList[i], "filtered"))
    #             continue
    ...
    #                 send_rst = srp(
    #                     Ether(src=self.sourceMAC, dst=targetMAC) / IPv6(src=selfIP, dst=targetIP) / TCP(sport=sport, dport=self.portList[i], flags="R"),
    #                     timeout=1,
    #                     verbose=0,
    #                     iface=self.iface,
    #                 )
    #                 self.portRes.append((self.portList[i], "open"))
    #                 continue

    #             elif resp.getlayer("TCP").flags == 0x14:
    #                 self.portRes.append((self.portList[i], "closed"))
    #                 continue
    ...
    #         self.finishedNMAP = True
    #     self.running = False
    #     sendp(Ether(src=self.sourceMAC, dst=self.sourceMAC), iface=self.iface, verbose=0)
    #     print(f"INFO (PEV) : NMAP Stopped on port {self.portList[self.lastPort]} | {self.lastPort+1}/{maxPorts}")
    #     self.saveNMAPres()
